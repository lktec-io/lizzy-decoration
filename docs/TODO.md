...
| Status | Task | Priority | Module | Completed |
|---|---|---|---|---|
| ☐ | DB: `users`, `sessions`, `refresh_tokens`, `password_resets` tables + migrations | Critical | Auth | |
| ☐ | Backend: register password hashing (bcrypt, cost 12) | Critical | Auth | |
| ☐ | Backend: login endpoint — JWT access token + refresh token (httpOnly cookie) | Critical | Auth | |
| ☐ | Backend: refresh-token rotation endpoint | Critical | Auth | |
| ☐ | Backend: logout current device / logout all devices | Critical | Auth | |
| ☐ | Backend: account lockout after 5 failed attempts / 15 min | High | Auth | |
| ☐ | Backend: forgot password → email token → reset password flow (Nodemailer) | Critical | Auth | |
| ☐ | Backend: device/session tracking on login (IP, user agent) | Medium | Auth | |
| ☐ | Frontend: Login page (logo, email, password, remember me, forgot password link, loading state, validation) | Critical | Auth | |
| ☐ | Frontend: Forgot Password / Reset Password pages | Critical | Auth | |
| ☐ | Frontend: Session Expired page + auto token refresh via Axios interceptor | High | Auth | |
| ☐ | Frontend: `AuthContext` + `ProtectedRoute` | Critical | Auth | |
| ☐ | Business rule: block login for inactive/deleted/suspended users | Critical | Auth | |
| ☐ | Quality Check: auth flows tested end-to-end, build/lint pass | Critical | Auth | |
