...
// phone carries a UNIQUE KEY (006_create_sales_pos.sql) — the only realistic
// conflict a customer create can hit (customer_code is generated by us and
// always unique by construction). Checked before generateCode() is called
// so a duplicate-phone attempt doesn't burn a sequence number it never uses.
export async function findByPhone(phone, excludeId = null) {
  const [rows] = await pool.query(
    `SELECT id, phone FROM customers WHERE deleted_at IS NULL AND phone = ? ${excludeId ? 'AND id <> ?' : ''} LIMIT 1`,
