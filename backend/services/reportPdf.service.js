...
function drawFooters(doc) {
  ...
  for (let i = 0; i < range.count; i += 1) {
    ...
    doc.fontSize(8).font('Helvetica').fillColor(COLOR_MUTED)
      .text('Generated by JOZZY Sales Management System', PAGE_MARGIN, bottom, { width: CONTENT_WIDTH, align: 'center' });
    doc.fontSize(8).font('Helvetica').fillColor(COLOR_MUTED)
      .text(`Page ${i + 1} of ${range.count}`, PAGE_MARGIN, bottom + mm(4), { width: CONTENT_WIDTH, align: 'center' });
  }
}
...
export function buildReportPdf(type, report, { dateFrom, dateTo, company, generatedByName, filtersLabel } = {}) {
  ...
  if (Array.isArray(report.analysis) && report.analysis.length > 0) {
    doc.fontSize(11).font('Helvetica-Bold').fillColor(COLOR_PRIMARY).text('Business Summary', { width: CONTENT_WIDTH });
    doc.moveDown(0.2);
    doc.fontSize(9).font('Helvetica').fillColor(COLOR_PRIMARY);
    report.analysis.forEach((line) => doc.text(`•  ${line}`, { width: CONTENT_WIDTH }));
    divider(doc);
  }
